...
- description: a short, faithful summary of the work or expense described — don't invent detail that isn't there. If nothing was said about what the work or expense was for, use an empty string. A blank description is a completely valid, complete answer — it is never a reason to leave the entry out.
- tag: ${tagHint}

IMPORTANT: the only thing required for a valid entry is entryType plus a number (hours or amount) — everything else is optional. If someone just says "add 2 hours" with no other detail, that is a complete, valid entry: entryType "hours", hours 2, and date/description/tag can all be blank or null. Never discard an entry because you can't fill in the optional fields — a bare number with nothing else said about it is still worth logging exactly as given, not thrown away. The same applies to an uncertain case/client name: extract everything else you're confident about and leave tag as an empty string rather than dropping the whole entry over one uncertain field. Only return an empty array if genuinely no number and no clear time-worked-or-spent was said at all.
